...
class TransferModel(nn.Module):
    def __init__(self, model, pipeline_type, rot, base_trans, transform_hom, scale, odd_type, direction=None, radius=None):
    ...
    def update_model_param(self, rot=None, base_trans=None, direction=None, radius=None):
        ...
        self.base_trans = base_trans if base_trans is not None else self.base_trans
...
class GsplatRGBOrigin(nn.Module):
    ...
    def render_alpha(self, pose, scene_dict, eps_max=1.0):
        ...
        min_distance = torch.tensor(self.min_distance).to(device=depth.device, dtype=depth.dtype)
        depth = torch.maximum(depth,min_distance)
        ...
        Ms_pix_abs = torch.abs(Ms_pix)                # [1, N, 2, 2]
        Ms_pix_abs += 1e-8
        # Ms_pix_max = Ms_pix_abs.amax(dim=(-1, -2))    # [1, N]
        # scale_factor = torch.minimum( torch.ones_like(Ms_pix_max), max_allowed/Ms_pix_max)  # [1, N]
        # Ms_pix = Ms_pix *scale_factor[:, :, None, None]
        scale_factor = torch.minimum(torch.ones_like(Ms_pix_abs), max_allowed[:, :, None, None]/Ms_pix_abs)  # [1, N]
...
class GsplatRGB(nn.Module):
    def __init__(self, camera_dict, scene_dict_all, min_distance=0.1, max_distance=5.0, bg_color=None, eps2d=5.0, gs_batch=100):
    ...
    def render_alpha(self, pose, scene_dict, eps_max=1.0):
        ...
        # J_00 = fx * torch.ones_like(depth) # [1, N]
        # J_02 = -fx * us / depth 
        # J_11 = fy * torch.ones_like(depth)
        # J_12 = -fy * vs
